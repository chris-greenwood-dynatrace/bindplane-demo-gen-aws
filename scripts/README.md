...
`terraform`, AWS credentials in the environment (`aws configure` / `AWS_PROFILE` / `AWS_ACCESS_KEY_ID`),
`ssh`, `rsync` (optional, falls back to scp).
...
| `scripts/up.sh [--demo NAME]` | Spin up a demo on AWS (prompts to pick if --demo omitted) |
| `scripts/up.sh --demo NAME --skip-validate` | Skip static validation (not recommended) |
| `scripts/down.sh [--demo NAME]` | Drain collectors + destroy AWS infra |
| `scripts/ssh.sh` | SSH into the running demo instance |
| `scripts/ssh.sh -L 8080:localhost:8080` | SSH with port-forward |
| `scripts/logs.sh --demo NAME` | Tail all docker compose logs from the instance |
...
Drains collectors (frees BindPlane cap), then destroys all AWS resources for the run atomically.
